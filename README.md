...
- "이전 재생 위치부터 재생하시겠습니까?" 팝업에 자동으로 답합니다. 기본은 **아니오**(처음부터)이고, 팝업 설정에서 **예**(이어보기)로 바꿀 수 있습니다.
- 새 강의가 열렸는데 멈춰 있으면 재생을 시작합니다.
- 팝업에서 재생 속도(0.5x ~ 2.0x)를 고르면 재생이 시작된 뒤 Kollus 플레이어에 계속 적용합니다.

> 크롬은 페이지를 한 번도 클릭하지 않은 상태에서 소리 있는 자동 재생을 막습니다.
> 새로고침 직후처럼 재생이 막히면 오른쪽 아래 **▶ 재생** 알림을 한 번 눌러 주세요. 이후 강의는 자동으로 이어집니다.
...
| `popup.html/js` | 자동 넘김 켜기/끄기, 재생 속도, 이어보기 팝업 응답 선택 |
